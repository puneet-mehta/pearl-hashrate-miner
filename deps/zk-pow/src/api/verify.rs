...
pub fn verify_plain_proof(block_header: &IncompleteBlockHeader, plain_proof: &PlainProof) -> Result<()> {
    ...
    // DIAG (Jackpot debug): log the computed hash + inputs that produced it so an
    // offline reproducer can compare verifier-side hash vs miner-side hash.
    // Cheap to emit (a few hundred bytes per call); strip after the bug is found.
    eprintln!(
        "DIAG verify_plain_proof: computed_jackpot_hash={:02x?} a_noise_seed={:02x?} nbits={:#x} jackpot_transcript={:?}",
